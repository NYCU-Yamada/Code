...
vector<int> sh{0}, th{0};
int ss[MAXN], tt[MAXN];
vector<int> hcon(270, 0); // each alphabet contribution to hash value
...
void solve() {
    ...
    // hash for s
        for (int i = 0; i < T; ++i) {
            ...
            for(int j = 'a'; j <= 'z'; ++j) {
                hcon[j] = (hcon[j] * P[0]) % MOD[0];
                if(s[i] == j) hcon[j]  = (hcon[j] + 1) % MOD[0];
            }
        }
    }
    ...
    auto subtract_id = [&](char c) {
        --svis[c];
        sh[0] = (sh[0] - hcon[c] + MOD[0]) % MOD[0];
        //debug("sub", c, sh[0], hcon[c]);
    };
    
    int pt_1 = fpow(P[0], T - 1, MOD[0]);
    int pt_0 = fpow(P[0], T, MOD[0]);
    
    int ans = (sh == th);
    for (int i=T;i<S;++i) {
        int L = i - T;
        cnt[s[L]]--;
        sh[0] = (sh[0] - pt_1 + MOD[0]) % MOD[0];
        cnt[s[i]]++;
        ...
        for (int j='a';j<='z';++j) {
            hcon[j] = hcon[j] * P[0] % MOD[0];
            if(s[L] == j) hcon[j] = (hcon[j] - pt_0 % MOD[0] + MOD[0]) % MOD[0]; 
            if(s[i] == j) hcon[j] = (hcon[j] + 1) % MOD[0];
        }
        ...
        if (!SZ(app[s[L]])) {
            ...
            sh[0] = (sh[0] + hcon[s[L]] * (mex_id - svis[s[L]]) % MOD[0] + MOD[0]) % MOD[0];
            svis[s[L]] = mex_id;
            chmax(max_id, mex_id);
        }
        //debug(max_id, mex_id);
        ...
        if (!svis[s[i]]) {
            svis[s[i]] = max_id + 1;
            sh[0] = (sh[0] + hcon[s[i]] * (max_id+1)) % MOD[0];
        }
        }
